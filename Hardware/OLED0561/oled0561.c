...
void OLED_DISPLAY_CLEAR_0(void){//指定区域清屏操作
	u8 j,t;
	for(t=0xB0;t<0xB8;t++){	//设置起始页地址为0xB0
		I2C_SAND_BYTE(OLED0561_ADD,COM,t); 	//页地址（从0xB0到0xB7）
		I2C_SAND_BYTE(OLED0561_ADD,COM,0x11); //起始列地址的高4位
		I2C_SAND_BYTE(OLED0561_ADD,COM,0x09);	//起始列地址的低4位
		for(j=24;j<132;j++){	//整页内容填充
 			I2C_SAND_BYTE(OLED0561_ADD,DAT,0x00);
 		}
	}
}
